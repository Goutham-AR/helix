...
use helix_core::command_line::{self, Args, Flag, ParseMode, Signature, DEFAULT_SIGNATURE};
...
fn lsp_workspace_command(
    ...
    if args.is_empty() {
        ...
        // TODO: change parsing mode to Parameters and pass the arguments to the language
        // server?
...
fn debug_eval(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    if let Some(debugger) = cx.editor.debugger.as_mut() {
        ...
        let response = helix_lsp::block_on(debugger.eval(args[0].to_string(), Some(frame_id)))?;
...
fn debug_start(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    let mut args: Vec<_> = args.into_iter().map(|arg| arg.to_string()).collect();
...
fn set_option(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    let value_error = |_| anyhow::anyhow!("Could not parse value `{}`", value);
    ...
    *json_value = if json_value.is_string() {
        // JSON strings require quotes, so we can't .parse() directly
        Value::String(value.to_string())
    } else {
        value.parse().map_err(value_error)?
    };
    let config = serde_json::from_value(config).map_err(value_error)?;
...
pub const TYPABLE_COMMAND_LIST: &[TypableCommand] = &[
    ...
    TypableCommand {
        ...
        signature: Signature {
            positionals: (0, Some(1)),
            // TODO: change this to Parameters? That could allow passing the
            // `helix_lsp::lsp::Command`'s `arguments` field.
            parse_mode: ParseMode::Raw,
            ..DEFAULT_SIGNATURE
        },
    },
...
fn execute_command_line(
    ...
    if rest.trim().is_empty() && command.parse::<usize>().ok().is_some() {
...
pub(super) fn execute_command(
    ...
    // Note: the result handling eagerly converts the Error to a string because args parsing
    // errors may contain borrows of the `args` parameter.
    let args = if event == PromptEvent::Validate {
        let tokens = command_line::ArgsParser::new(args, cmd.signature.parse_mode, true)
            .collect::<Result<Vec<_>, _>>()
            .map_err(|err| anyhow!("'{}': {err}", cmd.name))?;
        let expanded_tokens = tokens
            .into_iter()
            .map(|token| expansion::expand(cx.editor, token))
            .collect::<anyhow::Result<Vec<_>>>()
            .map_err(|err| anyhow!("'{}': {err}", cmd.name))?;
        Args::from_tokens(expanded_tokens, cmd.signature, true)
            .map_err(|err| anyhow!("'{}': {err}", cmd.name))?
    } else {
        Args::parse(args, cmd.signature, false).map_err(|err| anyhow!("'{}': {err}", cmd.name))?
    };

    (cmd.fun)(cx, args, event)
}
...
fn complete_command_args(
    ...
    use command_line::{ArgsParser, CompletionState, Token, TokenKind};
    ...
    let mut parser = ArgsParser::new(prefix, command.signature.parse_mode, false);
    ...
    while let Some(token) = parser
        .next()
        .transpose()
        .expect("arg parser cannot fail when validation is turned off")
    {
        tokens.push(token);
    ...
    // Add a fake final token when the input is not terminated with a token. This simulates an
    // empty argument. At a high level what this does is cause completion on an empty value
    // whenever you type space/tab. For example if you say `":open README.md "` (with that
    // trailing space) you should see the files in the current dir - completing `""` rather than
    // completions for `"README.md"` or `"README.md "`.
    if is_last_token {
        tokens.push(Token::empty_at(prefix.len()));
    }

    // If tokens were empty we would've pushed a spacer above.
    let token = tokens.last().expect("tokens should always be non-empty");
    ...
    match token.kind {
        TokenKind::Raw | TokenKind::Quoted => {
            ...
            match args.completion_state() {
                CompletionState::Positional => {
                    let completer = command.completer_for_argument_number(args.len() - 1);
                    completer(editor, token.content.as_ref())
                        .into_iter()
                        .map(|(range, mut span)| {
                            // TODO: ideally completers would have a way to signal that a
                            // completion can be unterminated. For example completing a filename
                            // `"a b.txt"` with `:open 'a ` should provide `'a b.txt'` while
                            // completing a directory name like `"b c/"` should complete `'b c/`
                            // (no closing single quote).
                            if command.signature.parse_mode == ParseMode::Parameters
                                && token.kind == TokenKind::Raw
                            {
                                span.content = command_line::escape_completion(span.content);
                            }
                            let range = (range.start + offset + token.start_byte)..;
                            (range, span)
                        })
                        .collect()
                }
                CompletionState::Flag => {
                    let text = token.content.trim_start_matches('-');
                    let flags = command.signature.flags.iter().map(|flag| flag.name);

                    fuzzy_match(text, flags, false)
                        .into_iter()
                        .map(|(name, _)| {
                            ((offset + token.start_byte).., format!("--{name}").into())
                        })
                        .collect()
                }
                // Completers for flag arguments are not yet implemented.
                CompletionState::FlagArgument => Vec::new(),
            }
        }
        TokenKind::Expand => {
            complete_expand(editor, token.content.as_ref(), offset + token.start_byte)
        }
        TokenKind::VariableExpansion => complete_variable_expansion(token, offset),
        TokenKind::UnicodeExpansion | TokenKind::ShellExpansion => Vec::new(),
    }
}

fn complete_expand(_editor: &Editor, content: &str, offset: usize) -> Vec<ui::prompt::Completion> {
    use command_line::{ArgsParser, Token, TokenKind};
    ...
    while let Some(idx) = content[start..].find('%') {
        ...
        if content.as_bytes().get(idx + '%'.len_utf8()).copied() == Some(b'%') {
            ...
            let mut parser = ArgsParser::new(&content[idx..], ParseMode::Parameters, false);
            let token = parser
                .parse_percent_token()
            ...
            match token {
                ...
                // completions if the token is already terminated.
                Some(Token { is_terminated, .. }) if is_terminated => continue,
                // Currently only variable expansions are supported by completion:
                Some(
                    token @ Token {
                        kind: TokenKind::VariableExpansion,
                        ..
                    },
                ) => {
                    return complete_variable_expansion(&token, offset + idx);
                }
                _ => continue,
            }
        }
...
fn complete_variable_expansion(
    token: &command_line::Token,
    offset: usize,
) -> Vec<ui::prompt::Completion> {
    fuzzy_match(&token.content, expansion::VARIABLES, false)
        .into_iter()
        .map(|(name, _)| {
            // TODO: the offset is not correct here. We would need to parse the delimiter
            // again.
            ((offset + token.start_byte).., (*name).into())
        })
        .collect()
}
